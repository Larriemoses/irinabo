...
export async function addStaff(formData:FormData){const adminStaff=await requireStaff(["ADMIN"]);const parsed=schema.safeParse(Object.fromEntries(formData));if(!parsed.success)return;const db=createServerSupabase();if(!db)return;await db.from("company_staff").insert({organisation_id:adminStaff.organisationId,full_name:parsed.data.fullName,username:parsed.data.username,role:parsed.data.role,access_code_hash:await hash(parsed.data.accessCode)});revalidatePath("/staff");}
