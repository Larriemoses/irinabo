...
export default async function Dashboard() {
  ...
  return <main className="p-5 lg:p-8">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-[var(--muted)]">Wednesday, 16 September</p><h1 className="display mt-1 text-4xl text-[var(--forest)]">Garage overview</h1></div><button className="rounded-full bg-[var(--forest)] px-5 py-3 text-sm font-bold text-white">+ Create journey</button></div>
    ...
    <section className="mt-8 grid gap-6 xl:grid-cols-[1.15fr_.85fr]"><div className="rounded-3xl border border-black/5 bg-white p-6"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-[var(--leaf)]">Journey {displayedTrip.code}</p><h2 className="display mt-1 text-2xl">{displayedTrip.origin} → {displayedTrip.destination}</h2></div><StatusPill>{displayedTrip.status}</StatusPill></div><div className="mt-7 grid gap-4 border-y border-[var(--line)] py-5 sm:grid-cols-3"><div><p className="text-xs text-[var(--muted)]">Vehicle</p><p className="font-bold">{displayedTrip.vehicle}</p></div><div><p className="text-xs text-[var(--muted)]">Driver</p><p className="font-bold">{displayedTrip.driver}</p></div><div><p className="text-xs text-[var(--muted)]">Departure</p><p className="font-bold">{displayedTrip.departure}</p></div></div><div className="mt-5 flex items-center justify-between"><p className="text-sm text-[var(--muted)]">{storedTrip ? "Connected to Supabase · Fictional seed" : "Local fallback data"}</p><Link href="/journey/demo-tw204" className="text-sm font-bold text-[var(--leaf)]">View journey →</Link></div></div><div className="rounded-3xl bg-[var(--lime)] p-6"><p className="text-xs font-bold uppercase tracking-widest text-[var(--forest)]/60">Attention needed</p><p className="display mt-3 text-3xl text-[var(--forest)]">One case has no accepted owner.</p><p className="mt-3 text-sm leading-6 text-[var(--forest)]/70">Protected case IRN-204-031 has passed its safety coordinator deadline. Independent fallback is ready.</p><Link href="/protected" className="mt-6 inline-block rounded-full bg-[var(--forest)] px-5 py-3 text-sm font-bold text-white">Open protected inbox</Link></div></section>
